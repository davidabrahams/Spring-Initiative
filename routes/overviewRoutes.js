  ...
  Overview.find({}, function(err, data) {
  	...
  	if(data[0] == undefined) {
  		Overview.create({
		    ...
		    if (err) {
		      res.send(err);
		      return res.status(500).json({msg: 'Error initializing overview'});
		    }
		    res.json({overviewData: newData});
		});
  	} else res.json({overviewData: data[0]});
  });
};
...
routes.POSTeditOverview = function(req, res) {
	...
	Overview.findOneAndUpdate({
		...
		if (err) {
			res.send(err);
			return res.status(200).json({msg: 'Error editing overview data'});
		} else res.json({overviewData: data});
	});
};
